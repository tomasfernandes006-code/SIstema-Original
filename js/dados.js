...
/* ---------------------------------------------------------------------
   ...
   sistema.html) e é a ÚNICA fonte de dados dos alunos: nenhum aluno
...
async function carregarAlunosDoArquivo() {
  const caminhos = [
    ARQUIVO_ALUNOS,
    "./" + ARQUIVO_ALUNOS,
    new URL(ARQUIVO_ALUNOS, window.location.href).href, // caminho absoluto na mesma pasta do sistema.html
  ];
  ...
  try {
    ...
    console.error(
      "Não foi possível ler o arquivo alunos.json.\n" +
        "Confira se o arquivo existe na MESMA pasta do sistema.html.\n" +
